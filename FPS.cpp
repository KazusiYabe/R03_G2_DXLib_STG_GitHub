...
VOID FPSUpdate(VOID)
{
	...
	{
		//計測開始時刻をマイクロ秒単位で取得
		fps.StarTime = GetNowHiPerformanceCount();
	...
	else
	{
		...
		//現在の平均FPS値が出る
		fps.DrawValue = 1000000.f / ((fps.NowTime-fps.StarTime) / (float)fps.SampleRate);

		//測定開始時刻をマイクロ秒単位で習得
		fps.StarTime = GetNowHiPerformanceCount();   //windowsが起動してから経過した時間(マイクロ秒)
...
VOID FPSWait(VOID)
{
	//現在の時刻-最初の時刻で、現在かかっている時刻を習得する
	LONGLONG resultTime = fps.NowTime - fps.StarTime;
